...
Generates a structured-JSON morning brief using DeepSeek, Gemini, and/or OpenAI.
...
Outputs:
  <out-dir>/morning_brief_deepseek.json  (structured shape per prompt.md)
  <out-dir>/morning_brief_gemini.json
...
Required env vars (at least one):
  DEEPSEEK_API_KEY  — DeepSeek V4 Flash via Responses API + web search
  GEMINI_API_KEY    — Google Gemini 3.7 Flash with Search Grounding
  OPENAI_API_KEY    — Codex tab (stored as provider=openai) with web_search_preview

Claude is subscription-only and is generated by claude_sdk_runner.ts instead.
"""
    ...
    for attempt in range(3):
        try:
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                f.write(body)
            print(f"[DeepSeek] Written to {out_path}")
            return True
...
def generate_gemini(prompt: str, out_dir: str) -> bool:
    ...
    print("[Gemini] Calling Gemini 3.7 Flash with Search Grounding...")
    ...
    meta = {
        ...
        "providers": {
            "deepseek": {
                ...
                "generated": results.get("deepseek", False),
...
def main():
    ...
    parser.add_argument(
        ...
        help="Comma-separated API providers to run (deepseek,gemini,openai)",
    )
    ...
    if "deepseek" in enabled:
        results["deepseek"] = generate_deepseek(prompt, out_dir)
    ...
    if not any(results.values()):
        ...
        print("  DEEPSEEK_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY")
        sys.exit(1)
