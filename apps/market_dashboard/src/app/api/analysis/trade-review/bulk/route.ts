...
export async function POST(request: Request) {
  try {
    ...
    const remainingQuota = Math.max(0, user.dailyScansLimit - user.dailyScansUsed);
    if (remainingQuota <= 0) {
    ...
    for (const trade of candidates) {
      try {
        ...
        const score = typeof verdict.review.overall_score === "number" ? verdict.review.overall_score : null;
        reviewed.push({
          id: trade.id,
          ticker: trade.ticker,
