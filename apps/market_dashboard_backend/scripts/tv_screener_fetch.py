...
5 hits per screener (cheap, ~20 calls/day total).

Outputs:
  data/tv_screeners.json — { fetched_at, screeners: [ { id, name, hits[] } ] }

Usage:
  python apps/market_dashboard_backend/scripts/tv_screener_fetch.py [--out-dir data] [--score]
...
def fetch_screener(screener_cfg: dict, columns: list) -> list:
    ...
    try:
        ...
        print(f"[tv:{screener_cfg['id']}] HTTP {e.code}: {msg}")
        return []
    except Exception as e:
        print(f"[tv:{screener_cfg['id']}] error: {e}")
        return []
    ...
    for row in data.get("data", []):
        ...
        d = row.get("d", [])
        # Map columns to values by index
...
def main():
    ...
    parser.add_argument("--score", action="store_true",
                        help="Auto-score top 5 hits per screener with DeepSeek")
    parser.add_argument("--score-top", type=int, default=5)
