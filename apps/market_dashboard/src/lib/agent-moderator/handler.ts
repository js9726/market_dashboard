/**
 * agent-moderator skill — local Next.js mirror.
 *
 * Canonical source: packages/core-skills/agent-moderator/handler.ts
 * This file is a verbatim copy kept inside the Next.js project root so
 * webpack/Turbopack can compile it (files outside the project root are
 * not transpiled by Next.js). Keep in sync when the skill is updated.
 */
